...
def render_metric_card(title, score):
    """Render a stylized metric card. Uses st.html() for reliable rendering."""
    ...
    <div style="background:rgba(15,23,42,0.7);border:1px solid rgba(255,255,255,0.08);
        ...
        <div style="width:100%;background:#1E293B;height:10px;border-radius:10px;
                    overflow:hidden;margin-bottom:12px;">
            <div style="width:{score}%;background:linear-gradient(90deg,{color},#3B82F6);
                        height:100%;border-radius:10px;"></div>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;">
            <span style="font-size:0.78rem;color:#94A3B8;">Status</span>
            <span style="background:rgba(255,255,255,0.05);border:1px solid {color};
...
def render_metrics_grid(metrics):
    """Render a 2-column grid of all core fitness metrics."""
    metric_labels = [
