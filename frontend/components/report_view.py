...
def render_full_report_view(report, on_reset_callback=None):
    ...
    activity   = str(report.get("activity", "batting")).title()
    ...
    report_id  = report.get("id", "REP-001")
    ...
    if "shot_classification" in report:
        ...
        shot_lbl  = str(shot.get("label", "")).replace("_", " ").upper()
    ...
    summary_html    = (ai_summary or "Analysis completed.").replace("<", "&lt;").replace(">", "&gt;")
    technique_html  = (technique_analysis or "").replace("<", "&lt;").replace(">", "&gt;")
    technique_block = (
    ...
    if annotated_video_url:
        ...
        try:
            st.video(f"{BACKEND_URL.rstrip('/')}{annotated_video_url}")
        except Exception:
            st.warning("Annotated video ready but playback encountered an issue.")
    ...
    if exercises:
        ...
        for idx, ex in enumerate(exercises):
            with cols[idx % num_cols]:
                ex_name  = ex.get("exercise_name", f"Drill {idx+1}")
                target   = ex.get("target_area", "Biomechanics")
                sets_reps = ex.get("sets_and_reps", "3 sets x 10 reps")
                diff     = ex.get("difficulty", "Intermediate")
                how_imp  = ex.get("how_it_improves", "Improves kinetic chain power.")
                st.html(f"""
                <div style="background:rgba(15,23,42,0.85);border:1px solid rgba(6,182,212,0.25);
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                        <span style="background:rgba(6,182,212,0.15);color:#06B6D4;padding:2px 8px;
                                     border-radius:8px;font-size:0.75rem;font-weight:700;">{diff.upper()}</span>
    ...
    if how_following_improves:
        ...
        <div style="background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.35);
            ...
            <p style="color:#E2E8F0;font-size:0.9rem;line-height:1.6;margin:0;">{how_following_improves}</p>
        </div>
        """)
    ...
    if nutrition_plan and isinstance(nutrition_plan, dict):
        ...
        with nc1:
            ...
            <div style="background:rgba(15,23,42,0.7);border:1px solid rgba(255,255,255,0.08);
                ...
                <p style="color:#CBD5E1;font-size:0.85rem;line-height:1.5;margin:0;">
                    {nutrition_plan.get("pre_workout","Complex carbohydrates and light protein 90m before.")}</p>
            </div>
            """)
        with nc2:
            st.html(f"""
            <div style="background:rgba(15,23,42,0.7);border:1px solid rgba(255,255,255,0.08);
                ...
                <p style="color:#CBD5E1;font-size:0.85rem;line-height:1.5;margin:0;">
                    {nutrition_plan.get("post_workout","High protein + electrolytes within 45m.")}</p>
            </div>
            """)
        with nc3:
            st.html(f"""
            <div style="background:rgba(15,23,42,0.7);border:1px solid rgba(255,255,255,0.08);
                ...
                <h5 style="color:#06B6D4;margin:0 0 6px 0;font-size:0.9rem;font-weight:800;">
                    &#x1F4A7; HYDRATION &amp; FOODS</h5>
                <p style="color:#CBD5E1;font-size:0.85rem;line-height:1.5;margin:0;">
                    {nutrition_plan.get("hydration_strategy","500ml water with sodium before training.")}</p>
            </div>
            """)
    ...
    with col_act2:
        if pdf_url:
            st.html(f"""
            <a href="{BACKEND_URL.rstrip('/')}{pdf_url}" target="_blank" style="text-decoration:none;">
