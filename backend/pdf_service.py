...
def generate_pdf_report(report_data: dict) -> str:
    ...
    progress_label_style = ParagraphStyle(
        'ProgTitle',
        parent=styles['Normal'],
        fontName='Helvetica-Bold',
        fontSize=9,
    ...
    boost_label_style = ParagraphStyle(
        'BoostTitle',
        parent=styles['Normal'],
        fontName='Helvetica-Bold',
        fontSize=9,
    ...
    # Progress & Efficiency Comparison (vs previous sessions)
    progress_comp = _sanitize_for_pdf(report_data.get("progress_comparison"))
    if progress_comp:
        prog_cell = [
            Paragraph("PROGRESS &amp; EFFICIENCY COMPARISON:", progress_label_style),
            Paragraph(str(progress_comp), body_style)
        ]
        t_prog = Table([[prog_cell]], colWidths=[530])
    ...
    # Coach's Athletic Motivational Boost
    motivational_boost = _sanitize_for_pdf(report_data.get("motivational_boost"))
    if motivational_boost:
        boost_cell = [
            Paragraph("COACH'S ATHLETIC BOOST:", boost_label_style),
            Paragraph(f"<i>\"{motivational_boost}\"</i>", body_style)
        ]
        t_boost = Table([[boost_cell]], colWidths=[530])
    ...
    metrics = cast(Dict[str, Any], _sanitize_for_pdf(report_data.get("metrics") or {}))
    ...
    exercises = cast(List[Any], _sanitize_for_pdf(report_data.get("exercises", [])))
    if not exercises and "recommendations" in report_data:
        ...
        exercises = cast(List[Any], _sanitize_for_pdf(report_data.get("recommendations", [])))
    ...
    nutrition = cast(Dict[str, Any], _sanitize_for_pdf(report_data.get("nutrition_plan") or {}))
    ...
    story.append(HRFlowable(width="100%", thickness=0.5, color=colors.HexColor("#CBD5E1"), spaceBefore=6, spaceAfter=8))  # type: ignore[arg-type]
