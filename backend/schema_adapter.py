...
def adapt_to_unified_report(
    ...
    if activity == "batting" and "shot_classification" in raw_model_output:
        ...
        # These two come from the Groq LLM insights (not raw_model_output) —
        # previously missing here, so they never reached the PDF even though
        # the LLM/fallback layer produced them correctly.
        if "vo2max_and_speed_analysis" in llm_insights:
            report["vo2max_and_speed_analysis"] = llm_insights["vo2max_and_speed_analysis"]
        if "shuttle_improvement_plan" in llm_insights:
            report["shuttle_improvement_plan"] = llm_insights["shuttle_improvement_plan"]
