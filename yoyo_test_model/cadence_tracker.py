...
def _parse_level_shuttle(yoyo_level: Optional[str]):
...
def _lookup_level_reference(yoyo_level: Optional[str]):
...
class ShuttleCadenceTracker:
    ...
    def process_frame(self, frame_bgr: np.ndarray, timestamp_ms: Optional[int] = None) -> dict:
    ...
    def finalize(self, manual_input: dict, reference: Optional[dict] = None) -> dict:
        ...
        comparison: dict[str, bool | float | None] = {
            "meets_target": None,
            "gap_to_target": None,
        }
...
def analyze_video_file(video_path: str, manual_input: dict, reference: Optional[dict] = None,
